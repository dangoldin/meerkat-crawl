...
def analyze_user(user_id):
    if user_id not in profiles_done:
        ...
        if 'result' in u:
            user_id_to_username_map[user_id] = u['result']['info']['username']
...
if __name__ == '__main__':
    ...
    with open(os.path.join(OUT_DIR, 'user_map'), 'w') as f:
        for user_id, username in user_id_to_username_map.iteritems():
